...
class Command(BaseCommand):
    ...
    @transaction.atomic
    def update_legislator(self, leg_json, state_geotag, mode):
    ...
    @transaction.atomic
    def update_bill(self, bill_json, state_geotag, mode):
    ...
    def handle(self, *args, **options):
        ...
        for state in metadata:
            ...
            state_geotag = None
            try:
                state_geotag = cm.GeoTag.objects.get(feature_type="SP", name=state["name"])
            except:
                num_state_errors += 1
            ...
            for f in legislator_files:
                with open(workingdir+"/legislators/"+f, 'r') as leg_file:
                    ...
                    result = self.update_legislator(leg_json, state_geotag, mode)
                    if result == "update":
            ...
            for f in bill_files:
                with open(f, 'r') as bill_file:
                    ...
                    result = self.update_bill(bill_json, state_geotag, mode)
                    if result == "update":
                        num_updated_bills += 1
